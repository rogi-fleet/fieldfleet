...
/// Step 1 of the budget-to-document wizard.
/// Shows templates grouped by category and emits selection via callback.
class BudgetDocumentTypePicker extends StatefulWidget {
  final ValueChanged<DocumentTemplate> onTemplateSelected;
  final VoidCallback onCancel;
  final TemplateCategory? defaultCategory;

  const BudgetDocumentTypePicker({
    super.key,
  ...
  @override
  State<BudgetDocumentTypePicker> createState() =>
      _BudgetDocumentTypePickerState();
}

class _BudgetDocumentTypePickerState extends State<BudgetDocumentTypePicker> {
  final _templateService = ServiceLocator.documentTemplateService;
  final _searchController = TextEditingController();
  String _searchQuery = '';
  late final Set<TemplateCategory> _expandedCategories;

  @override
  void initState() {
    super.initState();
    if (widget.defaultCategory != null) {
      _expandedCategories = {widget.defaultCategory!};
    } else {
      _expandedCategories = Set.from(TemplateCategory.values);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        // Template list
        Expanded(
          child: StreamBuilder<Map<TemplateCategory, List<DocumentTemplate>>>(
            stream: _templateService.getTemplatesByCategory(workspaceId),
            builder: (context, snapshot) {
              ...
              final hasTemplates = TemplateCategory.values.any(
              ...
              return SingleChildScrollView(
                ...
                child: Column(
                  children: TemplateCategory.values.map((category) {
                    ...
                    return _buildCategorySection(category, filteredTemplates);
                  }).toList(),
                ),
              );
            },
  ...
  Widget _buildCategorySection(
    ...
    final isExpanded =
        _searchQuery.isNotEmpty || _expandedCategories.contains(category);
