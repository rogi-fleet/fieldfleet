...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Use service role client to look up user emails
    ...
    const wantsMentionEmail = async (
      ...
      try {
        ...
        if (error) {
          console.error(
            "effective_notification_pref RPC error",
            error,
          );
          return true;
        }
    ...
    const taskUrl = `${SITE_URL}/#/projects/${projectId}/tasks/${taskId}`;
    ...
    const previewText = `${senderName} mentioned you on ${taskTitle}`;
    ...
    for (const mentionedUser of users) {
      ...
      if (!(await wantsMentionEmail(mentionedUser.id))) {
        skippedCount++;
        console.log(
          `Skipping mention email for ${mentionedUser.email}: prefs disabled or workspace muted`,
        );
        continue;
      }
      ...
      const html = buildTaskFleetEmail({
        ...
        bodyHtml: `
          <p style="margin: 0 0 12px 0;"><strong>${safeSender}</strong> mentioned you on <strong>${safeTaskTitle}</strong>.</p>
      ...
      try {
        ...
        const emailMessage = emailError instanceof Error
          ? emailError.message
          : String(emailError);
        console.error(`Error sending to ${mentionedUser.email}:`, emailError);
