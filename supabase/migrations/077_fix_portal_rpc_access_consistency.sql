-- Follow-up hardening for portal RPC authorization consistency.
-- Ensures inactive customers cannot access portal project/invoice reads.
-- Also uses positional parameters in filters to avoid any parameter/column ambiguity.
...
DECLARE
  normalized_email TEXT := LOWER(TRIM(COALESCE(auth.jwt() ->> 'email', '')));
BEGIN
  IF normalized_email = '' OR $1 IS NULL THEN
    RETURN;
  END IF;
